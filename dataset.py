...
def read_batch_online(X, Y, max_file_num, train_or_test):
    num_samp = X.shape[0]
    Xmat, Ymat = eng.online_image_generator(
        num_samp, -1, numpix_side, os.environ['LOCAL_SCRATCH'], nargout=2)
...
def make_real_noise(Fmap):
    ...
    A = np.concatenate((Fmap[0:Npix_2, Npix_2p2 - 1:],
                        np.conj(np.fliplr(np.flipud(Fmap[Npix_2p1 - 1, 1:Npix_2].reshape((1, -1)))))), axis=0)
    B = np.concatenate((Fmap[0:Npix_2p1, 0:Npix_2p1], A), axis=1)
    C = np.concatenate((np.zeros((Npix_2m1, 1)), np.conj(np.fliplr(np.flipud(
        Fmap[1:Npix_2, Npix_2p2 - 1:]))), np.conj(np.fliplr(np.flipud(Fmap[1:Npix_2, 1:Npix_2p1])))), axis=1)
...
def add_gaussian_noise(im):
    ...
    if np.random.uniform(low=0, high=1) <= 0.25:
        ...
        FFT_NOISE = np.random.normal(loc=0.0, scale=np.abs(R_n)) + \
            np.random.normal(loc=0.0, scale=np.abs(I_n)) * 1j
...
def apply_psf(im, my_max_psf_rms, apply_prob=1.0):
    ...
    if np.random.uniform() <= apply_prob:
        ...
        blurred_im = scipy.ndimage.filters.gaussian_filter(
            im.reshape(numpix_side, numpix_side), psf_rms)
...
def add_cosmic_ray(im, apply_prob=1):
    rand_state = np.random.get_state()
    if np.random.uniform() <= apply_prob:
        inds_cr = np.random.randint(9999)
        filename_cr = CRay_data_path + 'cosmicray_' + "%07d" % (inds_cr + 1) + '.png'
        CR_MAP = np.array(Image.open(filename_cr), dtype='float32').reshape(
            numpix_side * numpix_side,) / 255.0
...
def pick_new_lens_center(ARCS, Y, xy_range=0.5):
    rand_state = np.random.get_state()
    while True:
        x_new = np.random.randint(-1 * np.ceil(xy_range / 2 / pix_res),
                                  high=np.ceil(xy_range / 2 / pix_res))
        y_new = np.random.randint(-1 * np.ceil(xy_range / 2 / pix_res),
                                  high=np.ceil(xy_range / 2 / pix_res))
        m_shift = - int(np.floor(Y[3] / pix_res) - x_new)
        n_shift = - int(np.floor(Y[4] / pix_res) - y_new)
        shifted_ARCS = im_shift(ARCS.reshape((numpix_side, numpix_side)),
